...
func (r *OpenStackDataPlaneNodeSetReconciler) manageRabbitMqUserFinalizers(
	...
	// For each cell, get the current username and ensure our finalizer is on the matching RabbitMQUser
	for _, cellName := range cellNames {
		username, err := deployment.GetNovaCellRabbitMqUserFromSecret(ctx, helper, instance.Namespace, cellName)
		if err != nil {
			Log.Info("Failed to get RabbitMQ username for cell", "cell", cellName, "error", err)
			continue
		}
		...
		Log.Info("Found current RabbitMQ username for cell", "cell", cellName, "username", username)
	}
	...
	for i := range rabbitmqUserList.Items {
		...
		// Check if this user is currently in use by this nodeset
