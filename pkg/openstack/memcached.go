...
func reconcileMemcached(
	...
	if instance.Spec.TLS.PodLevel.Enabled {
		...
		// mTLS cert
		if instance.Spec.Memcached.SslVerifyMode != "" && instance.Spec.Memcached.SslVerifyMode != "None" {
			...
			certRequest = certmanager.CertificateRequest{
				...
				Hostnames: []string{
					fmt.Sprintf("*.openstack.svc"),
					fmt.Sprintf("*.openstack.svc.cluster.local"),
				},
				Labels: map[string]string{serviceCertSelector: ""},
	...
	op, err := controllerutil.CreateOrPatch(ctx, helper.GetClient(), memcached, func() error {
		...
		if tlsCert2 != "" {
			memcached.Spec.TLS.MTLS.SslVerifyMode = instance.Spec.Memcached.SslVerifyMode
