...
var _ = Describe("Dataplane NodeSet RabbitMQ Finalizer Management", func() {
	...
	Context("RabbitMQ User Credential Rotation", func() {
		...
		It("Should switch finalizer from old user to new user after rotation completes", func() {
			...
			// Give the controller time to reconcile and update node tracking
			// The nodeset controller needs to process the deployment completion
			// and update the service tracking ConfigMap
	...
	Context("When RabbitMQ users have cleanup-blocked finalizer", func() {
		...
		It("Should remove cleanup-blocked finalizer during nodeset deletion", func() {
			// Create SSH secret required by nodeset
			CreateSSHSecret(types.NamespacedName{Namespace: namespace, Name: "dataplane-ansible-ssh-private-key-secret"})

			// Create a simple nodeset - just need it to exist for testing deletion cleanup
			nodeSetSpec := map[string]interface{}{
				"preProvisioned": true,
				"services":       []string{"nova"},
				"nodes": map[string]dataplanev1.NodeSection{
			...
			// Wait for nodeset to be ready and have finalizerHash set
			Eventually(func(g Gomega) {
				nodeset := GetDataplaneNodeSet(dataplaneNodeSetName)
				g.Expect(nodeset.Status.FinalizerHash).ShouldNot(BeEmpty())
			}, timeout, interval).Should(Succeed())

			// Verify cleanup-blocked finalizer is still present on the user
			Eventually(func(g Gomega) {
				user := GetRabbitMQUser("nova-user1")
				g.Expect(user.Finalizers).Should(ContainElement(rabbitmqv1.RabbitMQUserCleanupBlockedFinalizer))
			}, timeout, interval).Should(Succeed())
